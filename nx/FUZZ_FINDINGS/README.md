...
| **MED-HIGH** | `clip` non-finite inconsistency | [clip_nonfinite_inconsistent.md](clip_nonfinite_inconsistent.md) | `clip(NaN, 0, 2)` returns `0.0` — NaN input silently becomes an in-range value; each argument position handles NaN differently, contradicting the min/max composition. Found 2026-08-17, convention-frontier probes. Pinned in `fuzz_nonfinite_convention_test.exs`. |
