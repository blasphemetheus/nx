// Fused Real-GRU Scan Backward Kernel
//
// Computes gradients for: h_t = (1 - z_t) * h_{t-1} + z_t * c_t
// where z_t = sigmoid(gates_t) (applied inside forward kernel from raw gates).
//
// This is the same recurrence as MinGRU but the forward kernel applies
// sigmoid in-kernel (MinGRU receives post-sigmoid z). The backward math
// is identical; the sigmoid chain rule is applied in Elixir.
//
// Reverse pass (T-1 -> 0):
...
//
// Note: Elixir applies: grad_gates = grad_z * z * (1 - z) (sigmoid derivative)
//
// Inputs:
//   z:            [B, T, H] — post-sigmoid gate values
//   candidates:   [B, T, H] — candidate values (passed through as-is)
...
__global__ void fused_real_gru_scan_backward_kernel(
...
int fused_real_gru_scan_backward_launch(
    cudaStream_t stream,
    ...
    fused_real_gru_scan_backward_kernel<<<grid, block, 0, stream>>>(
...
ffi::Error fused_real_gru_scan_backward_ffi_impl(
    cudaStream_t stream,
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    fused_real_gru_scan_backward, fused_real_gru_scan_backward_ffi_impl,
    ffi::Ffi::Bind()
...
XLA_FFI_REGISTER_HANDLER(XLA_FFI_GetApi(),
    "exla_fused_real_gru_scan_backward_" PRECISION_SUFFIX, "CUDA", fused_real_gru_scan_backward);
